...
def assign_tasks_to_participants(
    ...
    """Assign all tasks to participants, returning enriched task dicts.

    This is the public batch entry-point used by the API routes.
    Each returned dict contains the original task data plus:
        assignee, assignee_source, assignment_confidence
    """
    ...
    for task in tasks:
        ...
        # Run rule-based pre-check before the main pipeline
        desc = task_copy.get("description", "")
        rule_assignee, rule_source = _rule_assignee(desc, rules, participants)

        if rule_assignee and rule_source:
            # Explicit rule wins – highest confidence
            task_copy["assignee"] = rule_assignee
            task_copy["assignee_source"] = rule_source
            task_copy["assignment_confidence"] = 0.92
        else:
            assignee, source, confidence = assign_task_to_participant(
                task_copy, meeting_info, participants, rr_idx
            )
            if source == "round_robin":
                rr_idx += 1
            task_copy["assignee"] = assignee
            task_copy["assignee_source"] = source
            task_copy["assignment_confidence"] = confidence

        assigned.append(task_copy)
